import tokenColors from './theme/tokenColors/default.ts'
...
export async function generateAndLogTheme() {
    const uiFiles = fs.readdirSync(path.resolve(__dirname, './/theme//UIColors'))
        .filter(file => file.endsWith('.ts'));

    const uiModules = await Promise.all(
        uiFiles.map(file => import(pathToFileURL(path.resolve(__dirname, './/theme//UIColors', file)).href))
    );
    ...
    // write theme
    fs.promises.mkdir('./themes', { recursive: true })
        .then(() =>
            Promise.all([
