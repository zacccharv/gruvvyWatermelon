...
// src/theme/tokenColors/default.ts
var colors2 = colors_default.colors;
var tokenColors2 = colors_default.tokenColors;
var grammarTokens = [
  {
    scope: [
      "comment",
...
var default_default = { grammarColors };
...
var root = typeof __dirname === "undefined" ? path.dirname(fileURLToPath(import.meta.url)) : __dirname;
...
var configuration = () => {
  ...
  const defaultTagColorSetting = {
    "background": colors_default.gruvvyWatermelonColors.bg_1,
    "borderRadius": "0",
    "fontWeight": "bold",
    "foreground": colors_default.gruvvyWatermelonColors.black,
    "icon": "alert",
    "iconColour": colors_default.gruvvyWatermelonColors.bright_purple_pink,
    "type": "text"
  };
  const tagColorSettings = {
    "TODO": {
      "background": colors_default.gruvvyWatermelonColors.bright_aqua,
      "icon": "info"
    },
    "NOTE": {
      "background": colors_default.gruvvyWatermelonColors.bright_purple,
      "icon": "note"
    },
    "FIXME": {
      "background": colors_default.gruvvyWatermelonColors.red_pink,
      "icon": "flame"
    },
    "IMPORTANT": {
      "background": colors_default.gruvvyWatermelonColors.bright_red_pink,
      "icon": "star"
    }
  };
  return {
...
var updatePackageJson = async () => {
  return await readFile(path2.join(repoRoot, "package.json"), "utf8").then((data) => JSON.parse(data)).then((data) => {
    return {
      ...data,
      contributes: {
        ...data.contributes,
    ...
    writeFile(
      path2.join(repoRoot, "package.json"),
...
// node_modules/.pnpm/type-flag@4.0.0/node_modules/type-flag/dist/index.mjs
...
var U = a((t) => {
  ...
  for (const n in t) {
    ...
    d(e, n, r);
    const l = G(n);
    if (n !== l && d(e, l, r), "alias" in s && typeof s.alias == "string") {
...
var L = a((t, { onFlag: e, onArgument: n }) => {
  let s;
  const r = a((l, o) => {
...
var z = a((t, e = process.argv.slice(2), { ignore: n } = {}) => {
  const s = [], r = U(t), l = {}, o = [];
  return o[A] = [], L(e, { onFlag(i, c, f) {
    const g = (f.length === j || i.length > 1) && h(r, i);
    if (!n?.(g ? P : _, i, c)) {
      if (g) {
        const [y, p] = r[i], b = $(p, c), N = a((C, E) => {
          s.push(f), E && s.push(E), y.push(m(p, C || ""));
        }, "getFollowingValue");
        return b === void 0 ? N : N(b);
      }
      h(l, i) || (l[i] = []), l[i].push(c === void 0 ? true : c), s.push(f);
    }
  }, onArgument: a((i, c, f) => {
    n?.(D, e[c[0]]) || (o.push(...i), f ? (o[A] = i, e.splice(c[0])) : s.push(c));
  }, "onArgument") }), k(e, s), { flags: W(t, r), unknownFlags: l, _: o };
}, "typeFlag");
var H = a((t, e, n = process.argv.slice(2)) => {
  const s = new Set(t.split(",").map((c) => V(c)?.[0])), [r, l] = F(e), o = [], i = [];
  return L(n, { onFlag: a((c, f, w) => {
    if (!s.has(c) || !l && o.length > 0) return;
    const u = $(r, f), g = a((y, p) => {
      i.push(w), p && i.push(p), o.push(m(r, y || ""));
    }, "getFollowingValue");
    return u === void 0 ? g : g(u);
  }, "onFlag") }), k(n, i), l ? o : o[0];
}, "getFlag");

// src/index.ts
var __filename = fileURLToPath2(import.meta.url);
var __dirname2 = path3.dirname(__filename);
async function generateAndLogTheme() {
  const uiFiles = fs.readdirSync(path3.resolve(__dirname2, ".//theme//UIColors")).filter((file) => file.endsWith(".ts"));
  const uiModules = await Promise.all(
    uiFiles.map((file) => import(pathToFileURL(path3.resolve(__dirname2, ".//theme//UIColors", file)).href))
  );
  const uiObjects = import_lodash.default.merge({}, ...uiModules.map((module) => module.default));
  const mergedColors = {};
  function extractAndMergeColors(obj) {
    if (typeof obj === "object" && obj !== null) {
      if (obj.hasOwnProperty("colors") && typeof obj.colors === "object") {
  ...
  const gruvvyWatermelonTheme = {
    ...
    tokenColors: Object.values(default_default)[0].tokens
  };
  console.log("\nGenerated Theme:");
  console.log(JSON.stringify(gruvvyWatermelonTheme, null, 2));
  fs.promises.mkdir("./themes", { recursive: true }).then(
    () => Promise.all([
      fs.promises.writeFile("./themes/Gruvvy-Watermelon-color-theme.json", JSON.stringify(gruvvyWatermelonTheme, null, 2))
    ])
  ).catch(() => process.exit(1));
}
generateAndLogTheme().catch(console.error);
var shouldRegenerate = !H("--no-regenerate", Boolean);
if (shouldRegenerate) {
